...
1. Now, select the connection tab and enter the connection string details by getting the values from the output of the previous azdata endpoint list comman. You will see the following line in the previous output. Fetch the IP Address from the endpoint URL and add it in the connection string details.
